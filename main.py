...
def preprocess_data(data):
    ...
    stop_words = [
        ...
        '흥',
        '힘입어']
    vocab = list(filter(lambda x: len(x) > 1 and re.match(
        '^[가-힣]+$', x) and x not in stop_words, vocab))

    return titles, vocab


def build_tfidf_dtm(data, titles, vocab):
    dtm = lil_matrix((len(data), len(vocab)))

    for i in range(len(data)):
        d = data[i]['title']
        for j in range(len(vocab)):
            t = vocab[j]
            dtm[i, j] = tfidf(t, d, data)

    return dtm


def calculate_similarity(target_data, data, dtm):
    target_index = len(data) - 1  # target_data의 실제 인덱스
    target_vector = dtm.getrow(target_index)

    sim_scores = []
    for i in range(len(data)):
        if i != target_index and cosine_similarity(target_vector, dtm.getrow(i))[0][0] != 0:
            sim_scores.append({
                'title': data[i]['title'],
                'score': cosine_similarity(target_vector, dtm.getrow(i))[0][0]
            })
...
def practice(target_data):
    ...
    titles, vocab = preprocess_data(data)
    dtm = build_tfidf_dtm(data, titles, vocab)

    # 만들어진 TF-IDF DTM 출력
    print(f'* 영화 수: {dtm.shape[0]}, 단어 수: {dtm.shape[1]}')
    # print(dtm[:10].toarray())

    # target_data와 코사인 유사도를 구해서 가장 높은 순으로 정렬
    sim_scores = calculate_similarity(target_data, data, dtm)
